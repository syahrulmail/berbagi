...
<div class="page-header">
    <div>
        <h1><i class="fas fa-medal"></i> Tambah Pencapaian</h1>
        <p class="subtitle">Tambah kartu pencapaian baru.</p>
    </div>
...
<div class="card" style="max-width: 640px;">
    <form method="POST" action="{{ route('achievements.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" id="value" name="value" value="{{ old('value') }}" placeholder="Contoh: Rp 1,2 Miliar" required>
            <small style="color: var(--gray-500);">Angka/data yang ditampilkan mencolok pada kartu.</small>
        </div>
        <div class="form-group">
            <label for="label">Keterangan *</label>
            <input type="text" id="label" name="label" value="{{ old('label') }}" placeholder="Contoh: Dana wakaf & donasi tersalurkan" required>
            <small style="color: var(--gray-500);">Keterangan singkat di bawah data pencapaian.</small>
        </div>
        <div class="form-group">
            <label for="icon">Ikon (Font Awesome)</label>
            <input type="text" id="icon" name="icon" value="{{ old('icon') }}" placeholder="Contoh: fa-hand-holding-heart">
